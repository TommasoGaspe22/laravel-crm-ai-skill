...
Goal: feel as professional and information-dense as a polished enterprise CRM, but faster and simpler for non-technical sales users. Italian UI. Reuse the existing dashboard shell and CSS vocabulary — don't fork a new design language.

## Layout & shell
- Live inside `layouts/dashboard.blade.php` (existing sidebar + header + auth). Add a sidebar link **"CRM"** (or under a "Commerciale" group) with active state via `request()->routeIs('dashboard.crm.*')`, near "Oggi"/"Pipeline".
...
- Sticky first column (referente) is a nice-to-have, not required.
- Numeric/date columns right-or-left aligned consistently; truncate long text with title tooltip.
- Row is clickable to open detail (but checkboxes, links, and the action ▾ stop propagation).

## Badges (clear, scannable)
- **Stato**: colored pill per `crm_lead_statuses` (nuovo=neutral, contattato=info, nurturing=accent, appuntamento=warn/positive, vinto=positive/green, perso=danger/red, non_interessato=muted).
- **Priorità**: use `priority-{0..5}` classes; higher = stronger. Show 4–5 as "alta".
- **Lead score**: `score-fast/mid/low` (≥8 / ≥5 / else).
- **Urgenza / bucket** (from service): `overdue` = red with "in ritardo di N giorni", `today` = amber "oggi", `upcoming` = neutral, `first_contact` = accent "da contattare". These drive the strongest visual emphasis — an operator must spot overdue + today at a glance.
...
## States (all required)
- **Empty**: curated per view (e.g. "Nessun follow-up in ritardo. Ottimo lavoro." with a subtle icon and a link to another view). Never a blank table.
...
- **Missing/invalid data**: null follow-up date → "senza data"; null owner → "Non assegnato"; null email/phone → hide the quick link, don't render broken `mailto:`/`tel:`.
...
- Every interactive control has a visible label or `aria-label`; icon-only buttons get `aria-label` (mirror the reference org's "Mostra azioni", "Filtri", etc.).
...
- Checkbox column header labeled "Seleziona tutti".
...
## Copy & tone
- Italian, operational, imperative for actions ("Contatta", "Pianifica follow-up", "Registra esito"). Match the next-action phrasing already produced by the service ("Follow-up in ritardo da N giorni", "Contatta il lead via {canale}").
