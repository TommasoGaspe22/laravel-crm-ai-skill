...
Follow-up outcome constants (already defined): `APP. ANALISI FISSATO`, `NON HA RISPOSTO`, `NON INTERESSATO`, `NON ANCORA VALUTATO`, `APP. ANALISI IN ATTESA` (`FOLLOW_UP_OUTCOMES`). Sales outcome values in use: `VENDUTO`, `NON VENDUTO` (else pending).
...
Define the vocabulary once, in `config/crm.php` (extend the existing `lead_statuses` idea; the current keys are `new/contacted/qualified/converted/lost`). Use this CRM-specific set (superset, Italian labels):

```php
'crm_lead_statuses' => [
    'nuovo'           => 'Nuovo',            // not contacted
    'contattato'      => 'Contattato',       // working
    'nurturing'       => 'In nurturing',
    'appuntamento'    => 'Appuntamento',     // analysis/sales appointment set
    'vinto'           => 'Vinto',            // sales_outcome VENDUTO
    'perso'           => 'Perso',            // sales_outcome NON VENDUTO
    'non_interessato' => 'Non interessato',  // follow_up_outcome NON INTERESSATO
],
```

**Derivation (fallback when `status` is null)** — add an accessor `getComputedStatusAttribute()` on the model, priority order:
1. `sales_outcome === 'VENDUTO'` → `vinto`
2. `sales_outcome === 'NON VENDUTO'` → `perso`
3. `follow_up_outcome` or `second_follow_up_outcome === 'NON INTERESSATO'` → `non_interessato`
4. `analysis_appointment_set || sales_appointment_set` → `appuntamento`
5. `follow_up_outcome === 'APP. ANALISI IN ATTESA'` or any follow-up done → `nurturing`
6. `contacted === true` → `contattato`
7. else → `nuovo`

Rule: **explicit `status` wins; otherwise show computed.** Quick-action "cambia stato" writes the explicit `status`. This keeps old rows meaningful with zero backfill while letting the team override.

An "open lead" (for the default view) = `computed_status ∉ {vinto, perso, non_interessato}` — equivalently the existing Today-view predicate: `sales_outcome` null or not in `{VENDUTO, NON VENDUTO}` **and** not marked non-interested.

**Verified pitfall (SQL `NOT IN` vs. `NULL`):** because `status` is nullable and most rows will have `status = NULL` until an explicit "cambia stato" action runs, a saved-view filter written as `whereNotIn('status', ['vinto','perso','non_interessato'])` silently excludes every `NULL`-status row too — SQL's `NULL NOT IN (...)` evaluates to unknown, not true. That means a brand-new, never-touched lead disappears from its own default "Lead aperti" view. Confirmed by testing. Always pair it with an explicit `whereNull`: `$q->where(fn ($w) => $w->whereNull('status')->orWhereNotIn('status', [...]))`. Apply the same care anywhere else a saved view or filter is built against `status` directly instead of `computed_status` (see `saved-views-system.md`, `list-view-system.md`).
...
- "Assegnati a me" view = `where('owner_id', auth()->id())`.
...
- The controller must call `$this->authorize(...)` on show/update/quick-actions/bulk. List index filters by policy scope (all rows for staff; "assegnati a me" is a user-chosen filter, not a hard restriction, unless product wants sales to see only their own — if so, scope in `viewAny`).
...
Use the app timezone (`config('app.timezone')`) consistently. Follow-up buckets compare `startOfDay()` in that timezone (as the Today controller already does). Guard against null/invalid dates: null follow-up date → "senza follow-up" bucket, never a fatal error.
