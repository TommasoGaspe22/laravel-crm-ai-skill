...
### `crm_accounts`  (Account / Azienda — the hub)
`id, name*, website, type (cliente|prospect|partner|altro), description, parent_id(self-FK nullable), phone, billing_address, shipping_address, business_area, owner_id(FK users), source_entry_id(FK commercial_pipeline_entries nullable — provenance), timestamps, softDeletes`.
Model `CrmAccount`: `hasMany contacts`, `hasMany opportunities`, `hasMany activities (morph)`, `belongsTo parent`.

### `crm_contacts`  (Referente)
`id, account_id(FK crm_accounts nullable), first_name*, last_name*, title(qualifica), reports_to_id(self-FK nullable), email, phone, mailing_address, description, owner_id, source_entry_id, timestamps, softDeletes`.
Model `CrmContact`: `belongsTo account`, `belongsTo reportsTo`, `hasMany opportunities (as primary contact)`, morph activities.

### `crm_opportunities`  (Opportunità — value pipeline)
...
`qualifica=Qualifica, presentazione=Presentazione, proposta=Proposta, negoziazione=Negoziazione, vinta=Vinta, persa=Persa`.
`vinta`/`persa` are terminal (closed). Add `is_closed` / `is_won` accessors derived from stage. Default probability per stage is optional.
Model `CrmOpportunity`: `belongsTo account`, `belongsTo primaryContact`, `hasMany items`, morph activities.

### `crm_activities`  (Attività — Task/Call/Email/Event; replaces derived timeline)
...
## Lead conversion flow (reference "Converti")
...
3. Optionally create `crm_opportunities` (name default `"{company} — {oggi}"`, `stage=qualifica`, `close_date` = +30gg default, `amount` optional) linked to account + contact.
4. Copy `owner_id`; set the pipeline entry `status = convertito` (add to `crm_lead_statuses`) and stamp a `crm_activities` "convert" note.
...
## Access & policy
One policy per object (`CrmAccountPolicy`, `CrmContactPolicy`, `CrmOpportunityPolicy`, `CrmActivityPolicy`) following the Lead pattern in `data-model.md`: `admin` + `sales` can view/create/update; `delete` admin-only; ownership scoping for "Mie/Assegnati a me" views. Register all in the policy provider.

## Routing (mirror the Lead module)
Under the existing `auth` + `dashboard.` group, add `crm.` resources:
`/dashboard/crm/accounts`, `/contacts`, `/opportunities`, `/activities` (+ `/products`,`/price-books` later) — each `index` (reference-style list view) + `show` (detail) + quick-action `PATCH` endpoints. Bind route models to the `Crm*` models. Add a **Kanban by stage** for opportunities (SortableJS, deferred dependency) once stages are persisted — until then a "per fase" grouped list.
...
3. `crm_opportunities` (list + detail + **stage path**; "per fase" view).
4. Lead **Converti** action wiring the three together.
