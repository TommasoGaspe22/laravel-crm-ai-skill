...
description: Use when building, extending, or reviewing a Laravel CRM that needs an enterprise-style sales flow (Lead, Account, Referente/Contact, Opportunità/Opportunity with stages, Prodotti/Listini, Attività/Calendario) rebranded for your own app. Triggers on requests about /crm, list views, saved views, record detail pages, opportunity stages/pipeline, lead conversion, activities timeline, next-action queue, bulk/row actions, status/priority badges, or making a CRM feel like a polished enterprise product.
---
...
This skill is the implementation playbook for turning your Laravel app into an operational CRM that replicates a reference enterprise **sales flow**, rebranded for your own app. Scope (captured read-only from the live reference org): **Lead → (Converti) → Account + Referente + Opportunità → Prodotti/Listini → Attività/Calendario.** It captures the real reference UX, the data mapping, the reuse points in the existing code, and an incremental plan.
...
- Building Account / Referente / Opportunità (with stages) / Attività, or the Lead **Converti** flow.
...
**When NOT to use:** inbound funnel/assessment lead work lives in `App\Models\Lead` + `/dashboard/leads` (different table, keep separate). Enterprise-platform-only features (Flows, reports, permissions, AI assistant) — out of scope. Fatture/Invoices — out of scope for V1.
...
4. `references/data-model-full.md` — the downstream relational schema (`crm_accounts/contacts/opportunities/activities`…), stage enum, the **Converti** flow, per-object policies, and the whole-flow build order. Read for the *how* (data).
...
- **Auth + policy on every route and action.** `sales` and `admin` both use the CRM; "Assegnati a me" and ownership scoping go through the policy.
- **Italian UI**, the app's own design system, no external assets/CDNs beyond what the project already loads.
...
7. `crm_opportunities` with the real **stage path** (`Qualifica→Presentazione→Proposta→Negoziazione→Vinta/Persa`) + "per fase" view.
8. Lead **Converti** action → creates Account + Contact + Opportunità (transactional).
